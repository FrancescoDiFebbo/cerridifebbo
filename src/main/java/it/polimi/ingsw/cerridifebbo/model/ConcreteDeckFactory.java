...
public class ConcreteDeckFactory implements DeckFactory {
	...
	public Deck createDeck(int numberOfPlayers) {
		if (numberOfPlayers < CharacterDeckFactory.MIN_PLAYERS
				|| numberOfPlayers > CharacterDeckFactory.MAX_PLAYERS) {
			throw new IllegalArgumentException("Number of players between "
					+ CharacterDeckFactory.MIN_PLAYERS + " and "
	...
	private Deck getDeck(int numberOfPlayers) {
		List<Card> characterCards = new CharacterDeckFactory()
		...
		List<Card> escapeHatchCards = new EscapeHatchDeckFactory()
				.createDeck(null);

		return new Deck(characterCards, sectorCards, itemCards,
				escapeHatchCards);
	}
}
